  ...
  return (
    <div className="min-h-screen bg-background pb-32">
      ...
      <main className="py-6">
          ...
          <div className="flex overflow-x-auto no-scrollbar scroll-smooth gap-6 px-6 pb-8 -mt-2">
            {loadingDishes ? (
              ...
              return (
                <div 
                  key={dish.id} 
                  className="min-w-[320px] max-w-[320px] bg-white rounded-[40px] shadow-sm border border-black/5 flex flex-col p-5 group transition-all hover:shadow-2xl hover:shadow-primary/10 relative"
                >
                  <Link href={`/dish/${dish.id}`} className="relative h-72 w-full rounded-[32px] overflow-hidden mb-6 block">
                    <Image 
                  ...
                  <div className="flex-1 flex flex-col">
                    <h3 className="font-serif italic font-black text-2xl text-primary mb-1 uppercase tracking-tight line-clamp-1">{dish.name}</h3>
                    
                    <div className="flex items-center space-x-3 mb-4">
                      <div className="flex items-center space-x-1">
                        <Star className="h-4 w-4 text-amber-400 fill-current" />
                        <span className="text-[10px] text-muted font-bold">{dish.rating || "4.9"}</span>
                      </div>
                      <div className="flex items-center space-x-1">
                        <Clock className="h-4 w-4 text-muted" />
                        <span className="text-[10px] text-muted font-bold">{dish.prepTime || "25m"}</span>
                      </div>
                    </div>

                    <p className="text-[13px] text-muted font-medium leading-relaxed mb-6 line-clamp-2">
                      {dish.description || "Authentic delicacies prepared with the finest ingredients and traditional recipes."}
                    </p>
        ...
        {/* Additional horizontal sections can be added here (e.g. Popular, Recommendations) */}
      </main>
