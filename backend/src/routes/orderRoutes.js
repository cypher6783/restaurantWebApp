const express = require('express');
const orderController = require('../controllers/orderController');
const { protect, authorize } = require('../middlewares/authMiddleware');
const validate = require('../middlewares/validationMiddleware');
const { orderSchema } = require('../utils/validators');
const { mutationLimiter, apiLimiter } = require('../middlewares/rateLimiter');

const router = express.Router();

router.post('/', protect, mutationLimiter, validate(orderSchema), orderController.createOrder);
router.get('/my-orders', protect, apiLimiter, orderController.getMyOrders);
router.get('/:id', protect, apiLimiter, orderController.getOrder);

// Admin only
router.get('/', protect, authorize('ADMIN'), orderController.getAllOrders);
router.patch('/:id/status', protect, authorize('ADMIN'), orderController.updateStatus);

module.exports = router;
